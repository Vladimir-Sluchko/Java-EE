...
import airoportsinfo.utils.DateZonedUtils;
...
public class FlightsPoolDao implements IFlights {
    ...
    public List<Flights> get(FlightsFilter filter, Page page) {
        ...
        if (filter.getScheduledDeparture() != null) {
            ...
            where += " AND ? ";
            param.add(filter.getScheduledDeparture().withHour(23).withMinute(59).withSecond(59));
        }
        if (filter.getScheduledArrival() != null) {
            ...
            where += " AND ? ";
            param.add(filter.getScheduledArrival().withHour(23).withMinute(59).withSecond(59));
        }
        if (!where.isEmpty()) {
            where = "\n WHERE " + where;
        }
